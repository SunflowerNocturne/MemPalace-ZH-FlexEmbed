...
def test_chroma_collection_query_embeds_query_texts():
    fake = _FakeCollection()
    ...
    collection = ChromaCollection(fake, embedding_runtime=_FakeEmbeddingRuntime())
    result = collection.query(query_texts=["hello"], n_results=2)
    assert result == {"kind": "query"}
    assert fake.calls == [("query", {"query_embeddings": [[0.1, 0.2]], "n_results": 2})]
...
def test_chroma_backend_passes_embedding_function_when_configured(tmp_path):
    palace_path = tmp_path / "palace"
    ...
    fake_client = type(
        ...
        (),
        {
            ...
            "get_or_create_collection": lambda self, name, **kwargs: _FakeCollection(),
        },
    )

    with (
        patch("mempalace.backends.chroma.get_embedding_runtime", return_value=object()),
        patch("mempalace.backends.chroma.ChromaDocumentEmbeddingFunction", return_value=object()),
        patch("mempalace.backends.chroma.chromadb.PersistentClient", side_effect=fake_client),
    ):
        collection = ChromaBackend().get_collection(
            str(palace_path),
            collection_name="mempalace_drawers",
