...
class TestHandleRequest:
    ...
    def test_tools_call_preserves_utf8_text(self, monkeypatch, config, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from mempalace import mcp_server
        from mempalace.mcp_server import handle_request

        original_handler = mcp_server.TOOLS["mempalace_status"]["handler"]
        mcp_server.TOOLS["mempalace_status"]["handler"] = lambda: {"query": "上海 4天3晚"}
        try:
            resp = handle_request(
                {
                    "method": "tools/call",
        ...
        assert "上海 4天3晚" in text
        assert "\\u4e0a\\u6d77" not in text

    def test_preload_embeddings_warms_runtime(self, monkeypatch, config, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from mempalace import mcp_server
        ...
        assert warmed["texts"] == ["warmup"]

    def test_preload_embeddings_skips_when_disabled(self, monkeypatch, config, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from mempalace import mcp_server
