...
def test_extract_memories_decision_chinese():
    text = "我们决定改用 PostgreSQL，因为现在的方案扩展性太差，这次是一次明确的取舍。"
    result = extract_memories(text)
    assert len(result) >= 1
    assert any(m["memory_type"] == "decision" for m in result)


def test_extract_memories_problem_chinese():
    text = "现在有个严重问题，接口一直报错，根因是数据库连接池配置错误。"
    result = extract_memories(text)
    assert len(result) >= 1
    assert any(m["memory_type"] in {"problem", "milestone"} for m in result)


def test_extract_memories_emotional_chinese():
    text = "我感到很开心，也有点担心，但总体上我对这次突破非常骄傲。"
    result = extract_memories(text)
    assert len(result) >= 1
    assert any(m["memory_type"] == "emotional" for m in result)
