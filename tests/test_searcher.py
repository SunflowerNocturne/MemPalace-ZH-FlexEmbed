...
from mempalace.searcher import SearchError, _extract_query_tokens, search, search_memories
...
class TestSearchMemories:
    ...
    def test_search_memories_lexical_rerank_promotes_keyword_hit(self):
        """Exact keyword overlap should improve final ranking within recalled set."""
        mock_col = MagicMock()
        mock_col.query.return_value = {
            "documents": [[
                "Completely generic discussion with weak relevance.",
                "Neighborhood defense plan with exact mixed-language keyword matches.",
            ]],
            "metadatas": [[
                {"wing": "A", "room": "general", "source_file": "/tmp/a.md"},
                {"wing": "MemoryOps", "room": "architecture", "source_file": "/tmp/memory_ops.md"},
            ]],
            "distances": [[0.1, 0.2]],
        }

        with patch("mempalace.searcher.get_collection", return_value=mock_col):
            result = search_memories("memory plan defense", "/fake/path", n_results=2)

        assert result["results"][0]["wing"] == "MemoryOps"
        assert "memory" in result["results"][0]["matched_terms"]

    def test_search_memories_fetches_extra_candidates_for_rerank(self):
        mock_col = MagicMock()
        mock_col.query.return_value = {"documents": [[]], "metadatas": [[]], "distances": [[]]}

        with patch("mempalace.searcher.get_collection", return_value=mock_col):
            search_memories("test", "/fake/path", n_results=3)

        assert mock_col.query.call_args.kwargs["n_results"] == 20


def test_extract_query_tokens_supports_mixed_language():
    assert _extract_query_tokens("PRI 动作 列表 exercises moves") == [
        "pri",
        "动作",
        "列表",
        "exercises",
