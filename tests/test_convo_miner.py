import os
import tempfile
import shutil
from pathlib import Path
from unittest.mock import patch

import chromadb

from mempalace.convo_miner import detect_convo_room, mine_convos
from mempalace.palace import file_already_mined


class _FakeEmbeddingRuntime:
    """Deterministic offline embedding stub for convo mining tests."""

    def embed_documents(self, texts):
        return [[float(len(text)), float(text.count("memory"))] for text in texts]

    def embed_queries(self, texts):
        return [[float(len(text)), float(text.count("memory"))] for text in texts]


def test_convo_mining():
    tmpdir = tempfile.mkdtemp()
    try:
        with open(os.path.join(tmpdir, "chat.txt"), "w") as f:
            f.write(
                "> What is memory?\nMemory is persistence.\n\n> Why does it matter?\nIt enables continuity.\n\n> How do we build it?\nWith structured storage.\n"
            )

        palace_path = os.path.join(tmpdir, "palace")
        with patch("mempalace.backends.chroma.get_embedding_runtime", return_value=_FakeEmbeddingRuntime()):
            mine_convos(tmpdir, palace_path, wing="test_convos")

        client = chromadb.PersistentClient(path=palace_path)
        col = client.get_collection("mempalace_drawers")
        assert col.count() >= 2

        results = col.get(include=["documents"])
        docs = results["documents"]
        assert any("Memory is persistence." in doc for doc in docs)
        assert any("It enables continuity." in doc for doc in docs)
    finally:
        shutil.rmtree(tmpdir, ignore_errors=True)


def test_mine_convos_does_not_reprocess_short_files(capsys):
    """Files below MIN_CHUNK_SIZE get a sentinel so they are skipped on re-run."""
    tmpdir = tempfile.mkdtemp()
    try:
        # A file too short to produce any chunks
        with open(os.path.join(tmpdir, "tiny.txt"), "w") as f:
            f.write("hi")

        palace_path = os.path.join(tmpdir, "palace")

        # First run -- file is processed (sentinel written)
        with patch("mempalace.backends.chroma.get_embedding_runtime", return_value=_FakeEmbeddingRuntime()):
            mine_convos(tmpdir, palace_path, wing="test")
        capsys.readouterr()  # drain output

        # Verify sentinel was written (resolve path -- macOS /var -> /private/var)
        resolved_file = str(Path(tmpdir).resolve() / "tiny.txt")
        client = chromadb.PersistentClient(path=palace_path)
        col = client.get_collection("mempalace_drawers")
        assert file_already_mined(col, resolved_file)

        # Second run -- file should be skipped
        with patch("mempalace.backends.chroma.get_embedding_runtime", return_value=_FakeEmbeddingRuntime()):
            mine_convos(tmpdir, palace_path, wing="test")
        out2 = capsys.readouterr().out
        assert "Files skipped (already filed): 1" in out2
    finally:
        shutil.rmtree(tmpdir, ignore_errors=True)


def test_mine_convos_does_not_reprocess_empty_chunk_files(capsys):
    """Files that normalize but produce 0 exchange chunks get a sentinel."""
    tmpdir = tempfile.mkdtemp()
    try:
        # Content long enough to pass MIN_CHUNK_SIZE but with no exchange markers
        # (no "> " lines), so chunk_exchanges returns []
        with open(os.path.join(tmpdir, "no_exchanges.txt"), "w") as f:
            f.write("This is a plain paragraph without any exchange markers. " * 5)

        palace_path = os.path.join(tmpdir, "palace")

        with patch("mempalace.backends.chroma.get_embedding_runtime", return_value=_FakeEmbeddingRuntime()):
            mine_convos(tmpdir, palace_path, wing="test")
            mine_convos(tmpdir, palace_path, wing="test")
        out2 = capsys.readouterr().out
        assert "Files skipped (already filed): 1" in out2
    finally:
        shutil.rmtree(tmpdir, ignore_errors=True)


def test_mine_convos_chatbox_markdown_exports():
    tmpdir = tempfile.mkdtemp()
    try:
        with open(os.path.join(tmpdir, "chatbox.md"), "w", encoding="utf-8") as f:
            f.write(
                """# Chat

**system**:

```
You are a helpful assistant.
```

**user**:

```
请介绍一下Monika
```

**assistant**:

```
Monika是DDLC中的角色。
```

**user**:

```
继续展开
```

**assistant**:

```
她经常和第四面墙相关。
```
"""
            )

        palace_path = os.path.join(tmpdir, "palace")
        with patch("mempalace.backends.chroma.get_embedding_runtime", return_value=_FakeEmbeddingRuntime()):
            mine_convos(tmpdir, palace_path, wing="chatbox_test")

        client = chromadb.PersistentClient(path=palace_path)
        col = client.get_collection("mempalace_drawers")
        results = col.get(include=["documents", "metadatas"])
        docs = results["documents"]
        assert len(docs) == 2
        assert docs[0].startswith("> 请介绍一下Monika")
        assert "Monika是DDLC中的角色。" in docs[0]
        assert docs[1].startswith("> 继续展开")
        assert "她经常和第四面墙相关。" in docs[1]
    finally:
        shutil.rmtree(tmpdir, ignore_errors=True)


def test_detect_convo_room_chinese_technical():
    room = detect_convo_room("这个接口一直报错，我们需要调试代码并检查数据库连接。")
    assert room in {"technical", "problems"}
