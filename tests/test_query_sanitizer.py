...
class TestQuestionExtraction:
    ...
    def test_chinese_question_without_question_mark_uses_hint_words(self):
        system_prompt = "You are a helpful assistant. " * 50
        query = system_prompt + "请问我们为什么从 SQLite 切换到 ChromaDB"
        result = sanitize_query(query)
        assert result["was_sanitized"] is True
        assert "ChromaDB" in result["clean_query"] or "为什么" in result["clean_query"]
        assert result["method"] == "question_extraction"
