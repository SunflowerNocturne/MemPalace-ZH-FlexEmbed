...
def test_normalize_chatbox_markdown(tmp_path):
    f = tmp_path / "chatbox.md"
    f.write_text(
        """# Export
...
--------------------
<a href="https://chatboxai.app">Chatbox AI</a>
""",
        encoding="utf-8",
    )
    result = normalize(str(f))
    ...
    assert "> 你好，给我讲讲Monika" in result
    assert "当然可以，这是第一段回答。" in result
    assert "> 继续" in result
    assert "第二段回答。" in result
    assert "Chatbox AI" not in result


def test_try_chatbox_markdown_drops_empty_assistant():
    content = """# Export
...
<a href="https://chatboxai.app">Chatbox AI</a>
"""
    result = _try_chatbox_markdown(content)
    assert result is not None
    assert "> 第一问" in result
    assert "第一答" in result
