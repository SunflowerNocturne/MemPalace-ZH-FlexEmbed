...
def _maybe_preload_embeddings():
    """Warm the local embedding runtime for stdio MCP sessions.

    The first semantic search can otherwise pay model-load costs that some MCP
    clients surface as opaque tool failures. Preloading once at server startup
    keeps later tool calls predictable.
    """
    try:
        runtime = get_embedding_runtime(_config)
        if runtime is None:
            return
        start = time.time()
        runtime.embed_queries(["warmup"])
        logger.info("Embedding runtime ready in %.2fs", time.time() - start)
    except Exception:
        logger.exception("Failed to preload embedding runtime")
