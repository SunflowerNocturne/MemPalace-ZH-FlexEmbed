...
def normalize(filepath: str) -> str:
    ...
    chatbox_markdown = _try_chatbox_markdown(content)
    if chatbox_markdown:
        return chatbox_markdown
...
_CHATBOX_ROLE_RE = re.compile(r"^\*\*(system|user|assistant)\*\*:\s*$", re.I)


def _try_chatbox_markdown(content: str) -> Optional[str]:
    """Normalize Chatbox markdown exports into transcript format.
    ...
    We ignore system messages and drop empty assistant replies so incomplete
    exports do not create empty transcript turns.
    """
    lines = content.splitlines()
    if not any(_CHATBOX_ROLE_RE.match(line.strip()) for line in lines):
        return None

    messages = []
    i = 0
    while i < len(lines):
        match = _CHATBOX_ROLE_RE.match(lines[i].strip())
        if not match:
            i += 1
            continue
        ...
        if i < len(lines) and lines[i].strip().startswith("```"):
            ...
            while i < len(lines) and not _CHATBOX_ROLE_RE.match(lines[i].strip()):
                if lines[i].strip() == "--------------------":
                    break
                body_lines.append(lines[i])
                i += 1
